...
test('HWFramesConstraints should expose validSwFormats getter (darwin)', { skip: darwinFilter }, (t) => {
  t.ok(Array.isArray(constraints.validSwFormats))
})

test('HWFramesConstraints should expose validHwFormats getter (darwin)', { skip: darwinFilter }, (t) => {
...
test('HWFramesConstraints width constraints should be sensible (darwin)', { skip: darwinFilter }, (t) => {
  t.ok(constraints.minWidth >= 0)
  t.ok(constraints.maxWidth >= constraints.minWidth)
})

test('HWFramesConstraints height constraints should be sensible (darwin)', { skip: darwinFilter }, (t) => {
