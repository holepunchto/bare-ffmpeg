  ...
  constructor(io, options, url = defaultURL) {
    if (io instanceof IOContext) {
      ...
      try {
        ...
        // The native open failed (e.g. a JS IO callback threw). The io handle
        // was already transferred into this._io, so free it here before
        // rethrowing — otherwise the AVIOContext and its callback references
        // leak (the caller's original io is now a no-op empty handle).
        super.destroy()
        throw err
      }
