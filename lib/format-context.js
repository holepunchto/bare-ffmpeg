...
class FFmpegFormatContext {
  ...
  /**
   * Dumps details about all known streams in this format
   * the arguments control only printed Output(idx, url); they do nothing.
   */
  dump(printIdx = 0, printUrl = '') {
    binding.dumpFormatContext(this._handle, this._isOutput, printIdx, printUrl)
  }
...
exports.OutputFormatContext = class FFmpegOutputFormatContext extends (
  ...
  createStream() {
    const stream = new Stream(
      binding.createFormatContextStream(this._handle)
    )
