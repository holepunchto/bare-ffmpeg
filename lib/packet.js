...
module.exports = class FFmpegPacket {
  ...
  getSideDataByType(type) {
    const handles = binding.getPacketSideData(this._handle, type)
    if (!handles || !handles.length) {
      return null
    }

    return new SideData(handles[0])
  }

  addSideData(type, sideData) {
    const handle = binding.addPacketSideData(
      this._handle,
      type,
      sideData.buffer,
      sideData.byteOffset,
  ...
  [Symbol.for('bare.inspect')]() {
    const result = {
      ...
      hasNewExtradata: !!this.getSideDataByType(
        packetSideDataType.NEW_EXTRADATA
      ),
      hasParamChange: !!this.getSideDataByType(packetSideDataType.PARAM_CHANGE)
    }
